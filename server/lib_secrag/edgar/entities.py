...
class EntityData:
    ...
    @classmethod
    async def for_cik(cls, cik: int, include_old_filings: bool = True):
        return await get_entity_submissions(cik, include_old_filings=include_old_filings)
...
async def get_entity(entity_identifier: IntString, include_old_filings: bool = True) -> EntityData:
    """
    ...
    if is_ticker:
        return await EntityData.for_ticker(entity_identifier, include_old_filings=include_old_filings)
...
async def get_entity_submissions(cik: int,
    ...
    if os.getenv("EDGAR_USE_LOCAL_DATA"):
        ...
        submissions_json = await download_entity_submissions_from_sec(cik, include_old_filings=include_old_filings)
    if submissions_json:
        return parse_entity_submissions(submissions_json)



async def download_entity_submissions_from_sec(cik: int,
