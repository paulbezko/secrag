...
presenter_system_prompt = """
        You are given the latest user message and the messages from presenter, archivist, layout_changer, plotter AI agents.
        ...
        4. If the plotter output is empty, return concierge's, archivist's, or layout_changer's response as is without the [<widget_id>] placeholders or any mention of the plot.
        
        user_message: {user_message}
        concierge_response: {concierge_response}
        archivist_response: {archivist_response}
        plotter_response: {plotter_response}
        layout_changer_response: {layout_changer_response}
    """
...
async def presenter_node(state: State) -> Command[Literal["tailor"]]:
    ...
    last_layout_changer_message = get_last_node_message(state["messages"], "layout_changer")
    ...
    result = await presenter_chain.ainvoke({
        ...
        "layout_changer_response": last_layout_changer_message.content
    })
